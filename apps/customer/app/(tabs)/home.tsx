...
import { ItemCategoryLabels, type InventoryItem, type StoreProfile } from '@aks/shared';

const DEFAULT_LAT = 28.6315;
const DEFAULT_LNG = 77.2167;

async function fetchNearbyStores(): Promise<StoreProfile[]> {
  const res = await apiClient.get<{ data: StoreProfile[] } | StoreProfile[]>(
    `/api/v1/stores/nearby?lat=${DEFAULT_LAT}&lng=${DEFAULT_LNG}`
  );
  const payload = res.data as unknown;
  if (Array.isArray(payload)) return payload as StoreProfile[];
  return ((payload as { data?: StoreProfile[] }).data ?? []) as StoreProfile[];
}
  ...
  const itemsByStore = await Promise.all(
    stores.map(async (s) => {
      try {
        ...
        return ((payload as { data?: InventoryItem[] }).data ?? []) as InventoryItem[];
      } catch {
        return [];
      }
    })
