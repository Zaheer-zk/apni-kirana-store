...
export default function ChatScreen() {
  ...
  // 1) Resolve the chat (find/create the row, learn canSend)
  const chatQuery = useQuery({
    queryKey: ['chat', orderId],
    enabled: !!orderId,
    queryFn: async () => {
      const res = await apiClient.get(`/api/v1/chats/order/${orderId}`);
  ...
  // 2) Fetch messages once we have a chat id
  const messagesQuery = useQuery({
    queryKey: ['chat-messages', chatId],
    enabled: !!chatId,
    queryFn: async () => {
      const res = await apiClient.get(`/api/v1/chats/${chatId}/messages`);
  ...
  useLayoutEffect(() => {
    navigation.setOptions({ title: 'Order chat' });
  }, [navigation]);

  // 3) Socket subscription for real-time delivery
  useEffect(() => {
  ...
  const sendMutation = useMutation({
    mutationFn: async (body: string) => {
      const res = await apiClient.post(`/api/v1/chats/${chatId}/messages`, { body });
      return unwrap<ChatMessage>(res.data);
    },
    onMutate: async (body) => {
    ...
    onSuccess: (real, _body, ctx) => {
      if (!real || !ctx) return;
      // Swap optimistic with the real row (server-broadcast may also have
      // arrived via socket — dedupe by id either way)
